...
function CustomersPage({ user, showToast }) {
  ...
  const handleSave = async () => {
    ...
    if (editingCustomer) {
      ...
      if (!error) {
        // Sync any new tags to tag library
        if (form.tags?.length > 0) {
          const tagRows = form.tags.map(tag => ({ business_id: user.id, tag }));
          await supabase.from("tag_library").upsert(tagRows, { onConflict: "business_id,tag", ignoreDuplicates: true });
        }
        showToast(`${form.name} updated`, "success"); loadCustomers(); setShowModal(false);
      ...
      if (!error) {
        ...
        showToast(`${form.name} added`, "success"); loadCustomers(); setShowModal(false);
